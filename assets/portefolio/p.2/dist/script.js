...
new Vue({
  ...
  methods: {
  addNewTrack() {
    if (this.newTrackName && this.newTrackArtist && this.newTrackCover && this.newTrackSource) {
      this.tracks.push({
        name: this.newTrackName,
        artist: this.newTrackArtist,
        cover: this.newTrackCover,
        source: this.newTrackSource,
        favorited: false
      });
  ...
  createPlaylist() {
    if (this.newPlaylistName) {
      this.playlists.push({
        ...
        tracks: this.tracks // Vous pouvez ajouter une logique pour ajouter des pistes spécifiques
      });
      this.newPlaylistName = ''; // Reset du nom de playlist
    } else {
...
methods: {
  ...
  nextTrack() {
    if (this.isShuffling) {
      // Lecture d'un morceau aléatoire
